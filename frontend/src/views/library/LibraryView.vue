  ...
  <div class="min-h-screen" style="background:#020205;">
    ...
    <div class="max-w-7xl mx-auto px-4 mb-8">
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <div class="relative flex-1">
          ...
          <input
            ...
            class="w-full bg-white/[0.05] border border-white/10 text-white placeholder-white/25 text-base
                   rounded-xl ps-10 pe-4 py-3 focus:outline-none focus:border-brand/50 focus:ring-1 focus:ring-brand/30
        ...
        <select
          v-model="langFilter"
          @change="fetchBooks"
          class="bg-white/[0.05] border border-white/10 text-white text-base rounded-xl px-4 py-3
                 focus:outline-none focus:border-brand/50 transition-all duration-200 cursor-pointer"
      ...
      <div v-if="categories.length" class="flex flex-wrap gap-2 mt-4">
        <button
          ...
          :class="categoryFilter === '' ? 'bg-brand text-white border-brand' : 'bg-white/[0.04] text-white/50 border-white/10 hover:border-white/25'"
        ...
        <button
          ...
          :class="categoryFilter === cat ? 'bg-brand text-white border-brand' : 'bg-white/[0.04] text-white/50 border-white/10 hover:border-white/25'"
          class="text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-150"
        >
          {{ cat }}
        </button>
    ...
    <div class="max-w-7xl mx-auto px-4 pb-20">
      ...
      <div v-if="loading" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5">
        <div v-for="n in 10" :key="n" class="animate-pulse rounded-2xl overflow-hidden"
             style="background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07);">
          <div class="aspect-[3/4] bg-white/[0.06]"></div>
          <div class="p-4 space-y-2">
            <div class="h-3 rounded-full bg-white/[0.06] w-full"></div>
            <div class="h-3 rounded-full bg-white/[0.04] w-2/3"></div>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="books.length === 0" class="text-center py-24">
        ...
        <p class="text-white/40 text-lg">No books found.</p>
        <button v-if="searchQuery || categoryFilter || langFilter"
                @click="resetFilters"
                class="mt-4 text-brand-light text-sm hover:underline">
          Clear filters
        </button>
      </div>

      <!-- Grid -->
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5">
        <RouterLink
          v-for="book in books"
          :key="book.id"
        ...
        >
          ...
          <div class="aspect-[3/4] overflow-hidden relative"
            ...
            <!-- Audio indicator -->
            <div v-if="book.has_audio"
                 class="absolute top-2 end-2 w-6 h-6 rounded-full flex items-center justify-center"
                 style="background:rgba(35,78,204,0.8); backdrop-filter:blur(4px);"
          ...
          <div class="p-3 sm:p-4 flex flex-col flex-1">
            ...
            <span v-if="book.category"
                  class="inline-block text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full mb-2 self-start"
                  style="background:rgba(35,78,204,0.15); color:#3d65e8; border:1px solid rgba(35,78,204,0.25);">
              {{ book.category }}
            </span>

            <h3 class="text-white text-xs sm:text-sm font-semibold leading-snug line-clamp-2 mb-1 group-hover:text-brand-light transition-colors duration-200">
              {{ book.title }}
            </h3>
            <p class="text-white/35 text-[11px] truncate">{{ book.author || 'Unknown' }}</p>
            ...
            <div class="flex items-center gap-2 mt-auto pt-3">
              ...
              <span v-if="book.language && book.language !== 'en'"
                    class="text-[10px] font-semibold uppercase tracking-wider text-white/30 border border-white/10 rounded px-1.5 py-0.5">
                {{ book.language }}
              </span>
            </div>
          </div>
